...
$cartItemsIds = (count($cartItems) > 0) ? "" : -1;
foreach($cartItems as $key => $value) {
	$cartItemsIds .= $cartItems[$key];
	if ((int)$key < count($cartItems) - 1) {
		$cartItemsIds .= ',';
	}
}
$product_count = $db_handle->runQuery("SELECT COUNT(*) as count FROM tblproduct WHERE id IN ({$cartItemsIds})");
...
$product_array = $db_handle->runQuery("SELECT p.*, c.name as catName, MIN(pi.src) as imageSrc FROM tblproduct p {$tableCatJoin} {$tableImageJoin} WHERE p.id IN ({$cartItemsIds}) GROUP BY p.id");
...
<body class="wishlist">
    ...
    <div style="opacity: 0;" class="page-container slow-trans container-fluid">
        ...
        <div class="main-content no-bottom-marg row header-top">

			<div class="left-hand-nav col-md-3 p-2">
				<h4 class="mx-2 my-3">Search Products by Category</h4>
			...
			<div id="products-grid" class="col-md-9 pl-3">
				...
				if (count($cartItems) > 0) { ?>
					<div id="products-header">
						<h2>Your Wishlist<button type="button" class="book-appointment-btn btn btn-primary pull-right" onclick="showContactForm()"><i class="fa fa-clock-o"></i><span>Book Appointment</span></button></h2>      
						<p><span onclick="showContactForm()">Book an appointment</span> with us to talk about your saved Wishlist, and make your dreams a reality.</p> 
						<p class="pager-note">Showing products 1 - <?php echo $product_count[0]['count'] . ' of ' . $product_count[0]['count']; ?></p>
					</div>
					<div class="row products-container">
						<?php include_once('./templates/products-container.php'); ?>
					</div>
					<div id="products-bottom-header">
						<p>Showing products 1 - <?php echo $product_count[0]['count'] . ' of ' . $product_count[0]['count']; ?></p>
					</div>
				<?php }
				else { ?>
					<div id="products-header">
						<h2>Your Wishlist</h2>      
						<p>You don't currently have any products in your Wishlist. <a href="./products.php" title="Start browsing luxury furniture">Start browsing</a> now to find your next piece of hand-crafted luxury furniture.</p> 
					</div>
				<?php } ?>
			</div>
